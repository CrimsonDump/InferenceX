...
# 只在 config.json 的 `env` 里出现(调试/逃生阀): ARCH_RENAME(默认1) KV_CACHE_DTYPE
#   SERVER_WARMUP FLUSH_CACHE WATCHDOG_TIMEOUT SGLANG_DISABLE_RAGGED FLASHINFER_PREWARM
#   AR_FUSION MAX_RETRY
# 注意: 本脚本不知道也不需要知道【报哪套口径】(metric_mode) —— 那是报告端的事, 两套口径的
#   原始量 client 都会写进结果 JSON.
...
# "off" = 【不下发 --max-running-requests】, 用 sglang 自己按显存/上下文推的值 —— cookbook 的
# low-latency 那格命令里就没有这个 flag。此时后面夹 CUDA graph 上限用本次最大并发。
MAX_RUNNING_ARGS=()
MAX_RUNNING_OFF=0
case "$MAX_RUNNING_REQUESTS" in
    off|OFF|default) MAX_RUNNING_OFF=1; MAX_RUNNING_REQUESTS="$MAX_C";;
esac
...
[[ "$MAX_RUNNING_OFF" == "0" ]] && MAX_RUNNING_ARGS=(--max-running-requests="$MAX_RUNNING_REQUESTS")
...
#  - "off"(CUDA_GRAPH_MAX_BS=off) = 【不下发该 flag】, 走 sglang 默认(~160) —— cookbook 的三条
#    命令都没有这个 flag, 严格对齐时用它。注意 deepep 的 num_max_dispatch 也按它算, 见下方。
CUDA_GRAPH_MAX_BS="${CUDA_GRAPH_MAX_BS:-$MAX_C}"
CUDA_GRAPH_ARGS=()
CUDA_GRAPH_OFF=0
case "$CUDA_GRAPH_MAX_BS" in
    off|OFF|default) CUDA_GRAPH_OFF=1; CUDA_GRAPH_MAX_BS=160;;   # 160 = sglang 默认, 供下方推算用
esac
...
[[ "$CUDA_GRAPH_OFF" == "0" ]] && CUDA_GRAPH_ARGS=(--cuda-graph-max-bs="$CUDA_GRAPH_MAX_BS")
...
#   curve 的 mem_frac 优先 —— ★cookbook 三档正好相反(ll 0.8 / bal 0.85 / ht 0.85), 所以复现它时
#   必须逐曲线显式给★. (c=1024 无论如何装不下 KV, 需 offer>max-running 排队, 见 cookbook high-throughput.)
if [[ -n "${MEM_FRAC:-}" ]]; then :; elif [[ "$EP" -gt 1 ]]; then MEM_FRAC=0.80; else MEM_FRAC=0.85; fi

# context length: 默认 16384。★注意本地权重与官方权重的 max_position 都是 1048576(1M)★, 不设
# 这个 flag 时 sglang 就按 1M 算 KV池/并发上限 —— 而 cookbook 的三条命令【都没有这个 flag】,
# 所以要严格对齐 cookbook 就得 context_length="off"(不下发)。给具体数值则下发该值。
CONTEXT_LENGTH="${CONTEXT_LENGTH:-16384}"
CONTEXT_LEN_ARGS=()
case "$CONTEXT_LENGTH" in
    off|OFF|default|0|"") ;;
    *) CONTEXT_LEN_ARGS=(--context-length="$CONTEXT_LENGTH") ;;
esac

# chunked prefill: 默认 32768; curve 的 chunked_prefill_size="off"/0 则【不下发该 flag】
# (用 sglang 自己的默认, B200 上 16384) —— cookbook 的 low-latency / high-throughput 两格就是这样.
...
# client 侧 warmup 请求数(benchmark_lib.sh 自己的默认是 2*concurrency): 本项目默认 0 ——
# JIT/cudagraph 预热由下方 SERVER_WARMUP 块负责, 稳态口径又靠窗口刨 prefill, 不需要 warmup 请求.
# 复现 cookbook 时用 defaults.num_warmups=64 打开(= 它的 --warmup-requests 64).
...
if [[ -n "${BENCH_SGL_SRC:-}" ]]; then
    ...
    # ---- 先记下基座自带的版本号(必须在 editable 安装【之前】问, 之后就被盖住了) ----
    # ★为什么要记它★: 我们的源码树是 codeload tarball, 没有 .git, setuptools_scm 推不出版本
    # -> 装完 sglang.__version__ 恒为 "0.0.0"(见下面 _sgl_ver), 光看它无从知道跑的是哪个版本.
    # 而 commit 钉成基座自己那个 commit 时(当前 fdebc938), 基座的版本号就是这份源码的版本号
    # (已逐字节验证: 两棵树 2857 个共有 .py 全同, 只差镜像里那个生成的 _version.py).
    _base_sgl_ver="$(python3 -c 'import sglang,sys;sys.stdout.write(str(sglang.__version__))' 2>/dev/null)"
    echo "基座镜像自带 sglang 版本: ${_base_sgl_ver:-未知}"
    ...
    echo "sglang.__file__=$_sgl_file  __version__=$_sgl_ver (源码树无 .git 时恒为 0.0.0, 版本以 commit 为准)  sgl-kernel=$_kcur  基座版本=${_base_sgl_ver:-未知}"
    case "$_sgl_file" in
...
SERVER_ARGS=(
    ...
    ${MAX_RUNNING_ARGS[@]+"${MAX_RUNNING_ARGS[@]}"}
    ${CHUNKED_PREFILL_ARGS[@]+"${CHUNKED_PREFILL_ARGS[@]}"}
    ${CONTEXT_LEN_ARGS[@]+"${CONTEXT_LEN_ARGS[@]}"}
    # 不设时 sglang 默认 ~160 -> 超过它的并发退 eager 变慢; 故默认按本曲线最大并发下发.
    # cuda_graph_max_bs="off" 则不下发(= cookbook 的做法). dp-attn 的 >=DP 保护见上.
...
# KV dtype (= curve 的一等字段 kv_cache_dtype). sglang 侧【不给就是对的】: 改名后 DSA 自动
# 配置会选 fp8_e4m3(见 CLAUDE.md「DSA 路径」), 显式给反而可能与自动推导打架. 给值的两种场景:
# ① 自动选的 fp8 在某个 build 上翻车 -> kv_cache_dtype="bfloat16" 压回; ② 与别的 backend 做
# 严格同 dtype 对比. "off" 与不给等价(都是让 sglang 自己推).
if [[ -n "${KV_CACHE_DTYPE:-}" && "$KV_CACHE_DTYPE" != "off" && "$KV_CACHE_DTYPE" != "OFF" && "$KV_CACHE_DTYPE" != "default" ]]; then
...
if [[ "$EP" -gt 1 ]]; then
    ...
    # a2a 后端 = curve 的一等字段 all2all_backend, 不给则 deepep(= dep/tep 预设的定义, 也是
    # 一直以来的行为). "off" = 不下发, 退回 sglang 自己推. 取值是 backend 原生的名字 ——
    # sglang 这边是 deepep/none 之类, 与 vllm 的 deepep_low_latency/... 不是同一套字符串.
    SERVER_ARGS+=( --ep-size="$EP" )
    _a2a="${ALL2ALL_BACKEND:-deepep}"
    case "$_a2a" in
        off|OFF|default) echo "ALL2ALL_BACKEND=off: 不下发 --moe-a2a-backend";;
        *) SERVER_ARGS+=( --moe-a2a-backend="$_a2a" );;
    esac
fi
...
# 新版 sglang 有三条路会用 NVLink 多播 / torch 对称内存. 【坏 fabric 的节点上 8 卡 rendezvous
# 会死锁】—— 曾在 node071 上卡死 38 分钟无报错, 根因是 GPU6 的 18 条 NVLink 全掉出 fabric
# (2026-07-27 已用 `nvidia-smi -r -i 6` 修好, 三条路现在都走上游默认). 复发的判据与修法见
# CLAUDE.md「多播死锁的根因与修法」; 下面这两个开关只在来不及 reset 时临时用:
...
# REPS/MAX_RETRY 等)不影响 server 进程, 不在此列(它们记在 run_config.json). 供报告"测试配置"展示.
{
  ...
  if [[ -n "${BENCH_SGL_SRC:-}" ]]; then
    ...
    echo "# 实际生效: $(python3 -c 'import sglang,sys;sys.stdout.write(sglang.__file__)' 2>/dev/null)"
    # ★别把这里的 __version__ 当版本号★: tarball 源码树没有 .git, setuptools_scm 只能回退到
    # 0.0.0. 版本以 commit 为准; 基座自带版本一并记下(commit == 基座 commit 时两者就是同一份码).
    echo "#   sglang.__version__=$(python3 -c 'import sglang,sys;sys.stdout.write(str(sglang.__version__))' 2>/dev/null) (源码树无 .git -> setuptools_scm 回退, 不是真版本号)"
    echo "#   基座镜像(${BENCH_IMAGE_TAG:-未记录})自带 sglang 版本=${_base_sgl_ver:-未知}"
  else
  ...
  emit_env SGLANG_OPT_USE_CUSTOM_ALL_REDUCE_V2 "custom all-reduce 退回 v1(cudaIpc); v2 的对称内存 rendezvous 在坏 fabric 的节点上 8 卡死锁"
...
# 逐点的 client --num-prompts (curve.num_prompts, 与 batches 一一对应).
# 不给 -> num_prompts == 并发 = 等长齐发【一轮】(本项目一直的口径, 也是 cookbook fp8 页面的命令).
# ★给了就是另一种发压方式★: num_prompts > 并发 时请求是流水线补位的(一条跑完补一条),
#   稳态里始终有新请求在 prefill -> TPOT/TTFT 的统计构成与齐发一轮不同, 两者不可混比.
#   cookbook 的 nvfp4 复现命令就是这一类(c=1 发 8 条 / c=16 发 64 条).
IFS=',' read -ra NP_ARR <<< "${NUM_PROMPTS_LIST:-}"
_ci=-1
for c in "${CONC_ARR[@]}"; do
    ...
    np="${NP_ARR[$_ci]:-}"; np="${np// /}"
    [[ -z "$np" ]] && np="$c"
    [[ "$np" != "$c" ]] && echo "注意: c=$c 这一点用 --num-prompts $np (≠ 并发, 流水线补位发压)"
